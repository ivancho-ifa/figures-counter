...
BOOST_AUTO_TEST_CASE(EmptyInput) {
	...
	BOOST_REQUIRE_EQUAL(figures_counter::count_figures(0, 0, m_stream), 0);
}
	...
	for (auto row = 0; row < rows; ++row) {
		...
		if (row % 2 == 0) {
			for (auto col = 0; col < cols; ++col) {
				m_row += static_cast<char>(col % 2 == 0 ? figures_counter::Cell::Figure : figures_counter::Cell::Background);
			}
		}
		else {
